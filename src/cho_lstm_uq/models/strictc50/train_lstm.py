...
def build_argparser():
    ...
    ap.add_argument("--SPLIT_STRATEGY",
            ...
            choices=["auto","basic","pairs","rep_index","doe_holdout","doe_fixed"])
    ap.add_argument("--DOE_VAL_CORE", type=str, default="",
            help="Base DoE core (e.g. '50L_2_50L') to hold out as VAL when SPLIT_STRATEGY='doe_fixed'.")
...
def do_split(df, args):
    ...
    elif args.SPLIT_STRATEGY == "doe_fixed":
        # fixed 50L DoE core for VAL (e.g. 50L_2_50L)
        return train_val_split_doe_fixed(
            df,
            doe_val_core=getattr(args, "DOE_VAL_CORE", ""),
...
def run_tl_and_save(args):
    """Wrapper so external scripts can call this like a TL trainer."""
